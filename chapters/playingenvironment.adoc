...
The two figures below show the dimensions of the field, the goals and special field areas, measured in millimeters between the line centers.  <<field-dimensions-b, Figure 1>> for SSL-Regular.

NOTE: The numbers in the figures below show the distances in millimeters between the line centers.

[[field-dimensions-b]]
.Field dimensions and markings for SSL-Regular
...
===== Defense Area
A defense area is defined as a rectangle touching the goal lines centrally in front of both <<Goals, goals>>. The size of the defense area is 2 meters times 1 meter, as shown in figure <<field-dimensions-b, 2>> respectively.

===== Penalty Mark
The penalty mark defines the point from which a team executes a penalty kick against the opponent goal. It is located on the <<Goal-to-Goal Line, goal-to-goal line>> and 6 meters away from the opponent's goal center.
...
The distance between the side walls is 1 meter, and the goal is 0.18 meters deep. The goal walls are 0.02 meters thick and touch the goal line, but do not overlap or encroach on the field lines or the field. <<goal-detail-b, Figure 2>> show these details for SSL-Regular.
The goal side walls extend to the outside wall.

NOTE: The numbers in the figures below show the distances in millimeters.

[[goal-detail-b]]
.The goal in detail for SSL-Regular
